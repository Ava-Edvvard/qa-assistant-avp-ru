...
Перед началом проектирования пользователь может настроить предпочитаемую нейросеть (OpenAI, Gemini или кастомный совместимый эндпоинт) прямо на главной странице.
...
-   **Действие системы**: Строит матрицу трассируемости, сопоставляя требования (строки) со сценариями (столбцы), подсчитывает покрытие для каждого требования. Выводит готовые тест-кейсы в формате Markdown, удобном для копирования.
...
-   **Действие системы**: LLM сопоставляет старые тест-кейсы (из Шага 1) с обновленными (из Шага 4) и выводит структурированный отчет об изменениях (Diff Report) с разделением на добавленные, измененные и удаленные сценарии.

### Этап 6: Матрица трассируемости и экспорт
-   **Действие пользователя**: Просматривает и копирует финальную матрицу, новые тест-кейсы и отчет об изменениях из Шага 5.
