...
graph TD
    ...
    FastAPI -->|Запросы к LLM| OpenAI[Kaspersky LLM API (OpenAI-совместимый)]
...
- **Сервис работы с LLM (`app/services/llm_service.py`)**: Взаимодействует с Kaspersky LLM API. Поддерживает **динамическую инициализацию**, создавая клиентские подключения «на лету», когда кастомные учетные данные пользователя передаются в полезной нагрузке запроса, с переключением на переменные окружения сервера или локальный Mock-режим.
