...
class LLMService:
    ...
    def compare_scenarios(self, old_text: str, new_cases: List[TestScenario], llm_config: Optional[LLMConfig] = None) -> str:
        ...
        prompt = f"""
        Выступай в роли ведущего QA-инженера (QA Lead). Проведи детальный анализ и сравнение старого набора тест-кейсов/требований с новым сгенерированным набором тест-сценариев.
        
        ИСКЛЮЧИ любые вводные фразы (например, "Вот отчет...", "Ниже приведено...") и любые заключения/выводы в конце (например, "Этот отчет поможет...", "С уважением..."). Выводи ТОЛЬКО содержательные разделы отчета.
        
        Отчет должен строго состоять из трех разделов в формате Markdown:

        ### 1. Сводка по изменениям в требованиях
        Опиши, чем отличаются старые требования от новых (какие бизнес-требования были добавлены, изменены или исключены). Сделай это списком. Если информация не изменилась, напиши "Изменений в требованиях не обнаружено".
        ...
        Приведи точную сравнительную статистику по количеству тест-кейсов и их приоритетам в следующем формате (вместо X, Y, Z, A, B, C подставь реальные подсчитанные цифры):
